...
import { createR2UploadUrl, isR2Configured, settingsMediaKey } from '@/lib/r2'
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const key = settingsMediaKey(filename)
    ...
    return NextResponse.json({
      ...
      publicUrl: upload.publicUrl || '/api/settings/avatar',
    })
  } catch (error) {
    console.error('Admin settings presign:', error)
    return NextResponse.json(
      {
        success: false,
