...
export async function GET(_req: NextRequest, { params }: Params) {
  try {
    ...
    if (!tour || !tour.bannerImageKey) {
      return NextResponse.json({ success: false, error: 'Not found.' }, { status: 404 })
    }

    const publicUrl = publicUrlForKey(tour.bannerImageKey)
    if (publicUrl) return NextResponse.redirect(publicUrl, 302)

    const signed = await createR2DownloadUrl(tour.bannerImageKey)
    return NextResponse.redirect(signed, 302)
  } catch (error) {
    console.error('Tour banner GET:', error)
